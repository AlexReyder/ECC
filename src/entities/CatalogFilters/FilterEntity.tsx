...
const FilterEntity = ({ data, name }: { data: string[]; name: string }) => {
  const state: Record<string, boolean> = data.reduce(
    (a, v) => ({ ...a, [v]: false }),
    {},
  );
          ...
          return (
            <div key={rendered} className={s.FilterContainer}>
              <input
                ...
                value={slug(rendered)}
                onChange={(e) => handleChange(e, rendered)}
                checked={inputs[`${rendered}`]}
              />
              <label
                htmlFor={rendered}
                className={`${s.FilterText} ${inputs[rendered] ? s.FilterText__active : ""}`}
              >
                {name === "colors" ? rendered : rendered}
              </label>
            </div>
          );
        })}
